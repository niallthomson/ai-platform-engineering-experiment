...
class ModelConfig:
    ...
    def __init__(self, settings: LazySettings):
        ...
        self.model_id = settings.get("model.name", "us.anthropic.claude-sonnet-4-20250514-v1:0")
...
class AgentConfig:
    ...
    def __init__(self, settings: LazySettings):
        ...
        self.description = settings.get("description", "This is the default agent configuration")
        self.system_prompt = settings.get("system_prompt", "")
        
        self.model = ModelConfig(settings)
        
        self.mcp_servers = [MCPServer(**server) for server in settings.get("mcp_servers", [])]
